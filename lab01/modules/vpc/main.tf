locals {
  max_subnet_length = max(
    ...
    length(var.transit_subnets),
    length(var.appliance_subnets),
...
################################################################################
# Transit routes
# Subnet is for transit gateway attachment
# There are as many routing tables as the number of NAT gateways
################################################################################

resource "aws_route_table" "transit" {
  count = local.create_vpc && local.max_subnet_length > 0 ? length(var.transit_subnets) : 0
  ...
  tags = merge(
    {
      "Name" = format(
        "${var.name}-${var.transit_subnet_suffix}-%s",
        element(var.azs, count.index),
      )
    },
    var.tags,
    var.transit_route_table_tags,
  )
}

################################################################################
# Appliance routes
# Subnet dedicated for VPC appliance like network firewall
################################################################################

resource "aws_route_table" "appliance" {
  count = local.create_vpc && local.max_subnet_length > 0 ? length(var.appliance_subnets) : 0

  vpc_id = local.vpc_id

  tags = merge(
    {
      "Name" = format("${var.name}-appliance-%s", element(var.azs, count.index))
    },
    var.tags,
  )
}
...
resource "aws_subnet" "transit" {
  count = local.create_vpc && length(var.transit_subnets) > 0 ? length(var.transit_subnets) : 0

  vpc_id                          = local.vpc_id
  cidr_block                      = var.transit_subnets[count.index]
  availability_zone               = length(regexall("^[a-z]{2}-", element(var.azs, count.index))) > 0 ? element(var.azs, count.index) : null
  availability_zone_id            = length(regexall("^[a-z]{2}-", element(var.azs, count.index))) == 0 ? element(var.azs, count.index) : null
  assign_ipv6_address_on_creation = var.transit_subnet_assign_ipv6_address_on_creation == null ? var.assign_ipv6_address_on_creation : var.transit_subnet_assign_ipv6_address_on_creation

  ipv6_cidr_block = var.enable_ipv6 && length(var.transit_subnet_ipv6_prefixes) > 0 ? cidrsubnet(aws_vpc.this[0].ipv6_cidr_block, 8, var.transit_subnet_ipv6_prefixes[count.index]) : null
...
resource "aws_subnet" "appliance" {
  count = local.create_vpc ? length(var.appliance_subnets) : 0

  vpc_id               = local.vpc_id
  cidr_block           = var.appliance_subnets[count.index]
  availability_zone    = length(regexall("^[a-z]{2}-", element(var.azs, count.index))) > 0 ? element(var.azs, count.index) : null
  availability_zone_id = length(regexall("^[a-z]{2}-", element(var.azs, count.index))) == 0 ? element(var.azs, count.index) : null
...
################################################################################
# Transit Network ACLs
################################################################################

resource "aws_network_acl" "transit" {
  count = local.create_vpc && var.transit_dedicated_network_acl && length(var.transit_subnets) > 0 ? 1 : 0

  vpc_id     = local.vpc_id
  subnet_ids = aws_subnet.transit[*].id

  tags = merge(
    { "Name" = "${var.name}-${var.transit_subnet_suffix}" },
    var.tags,
    var.transit_acl_tags,
  )
}

resource "aws_network_acl_rule" "transit_inbound" {
  count = local.create_vpc && var.transit_dedicated_network_acl && length(var.transit_subnets) > 0 ? length(var.transit_inbound_acl_rules) : 0

  network_acl_id = aws_network_acl.transit[0].id

  egress          = false
  rule_number     = var.transit_inbound_acl_rules[count.index]["rule_number"]
  rule_action     = var.transit_inbound_acl_rules[count.index]["rule_action"]
  from_port       = lookup(var.transit_inbound_acl_rules[count.index], "from_port", null)
  to_port         = lookup(var.transit_inbound_acl_rules[count.index], "to_port", null)
  icmp_code       = lookup(var.transit_inbound_acl_rules[count.index], "icmp_code", null)
  icmp_type       = lookup(var.transit_inbound_acl_rules[count.index], "icmp_type", null)
  protocol        = var.transit_inbound_acl_rules[count.index]["protocol"]
  cidr_block      = lookup(var.transit_inbound_acl_rules[count.index], "cidr_block", null)
  ipv6_cidr_block = lookup(var.transit_inbound_acl_rules[count.index], "ipv6_cidr_block", null)
}

resource "aws_network_acl_rule" "transit_outbound" {
  count = local.create_vpc && var.transit_dedicated_network_acl && length(var.transit_subnets) > 0 ? length(var.transit_outbound_acl_rules) : 0

  network_acl_id = aws_network_acl.transit[0].id

  egress          = true
  rule_number     = var.transit_outbound_acl_rules[count.index]["rule_number"]
  rule_action     = var.transit_outbound_acl_rules[count.index]["rule_action"]
  from_port       = lookup(var.transit_outbound_acl_rules[count.index], "from_port", null)
  to_port         = lookup(var.transit_outbound_acl_rules[count.index], "to_port", null)
  icmp_code       = lookup(var.transit_outbound_acl_rules[count.index], "icmp_code", null)
  icmp_type       = lookup(var.transit_outbound_acl_rules[count.index], "icmp_type", null)
  protocol        = var.transit_outbound_acl_rules[count.index]["protocol"]
  cidr_block      = lookup(var.transit_outbound_acl_rules[count.index], "cidr_block", null)
  ipv6_cidr_block = lookup(var.transit_outbound_acl_rules[count.index], "ipv6_cidr_block", null)
}
...
resource "aws_nat_gateway" "this" {
  ...
  subnet_id = element(
    var.enable_network_firewall_routing ? aws_subnet.appliance[*].id : aws_subnet.public[*].id,
    var.single_nat_gateway ? 0 : count.index,
  )
...
resource "aws_route" "transit_to_nat_gateway" {
  count = local.create_vpc && var.enable_nat_gateway ? local.nat_gateway_count : 0
...
resource "aws_route" "appliance_to_firewall" {
  count = local.create_vpc && var.enable_network_firewall_routing ? length(var.appliance_subnets) : 0

  route_table_id         = element(aws_route_table.appliance[*].id, count.index)
  destination_cidr_block = var.network_firewall_destination_cidr_block
  vpc_endpoint_id        = element(var.network_firewall_endpoint_ids, count.index)
...
resource "aws_route" "transit_ipv6_egress" {
  count = local.create_vpc && var.create_egress_only_igw && var.enable_ipv6 ? length(var.transit_subnets) : 0
...
resource "aws_route_table_association" "appliance" {
  count = local.create_vpc && length(var.appliance_subnets) > 0 ? length(var.appliance_subnets) : 0

  subnet_id      = element(aws_subnet.appliance[*].id, count.index)
  route_table_id = element(
    aws_route_table.appliance[*].id,
    count.index,
  )
}
...
count = local.create_vpc && var.propagate_transit_route_tables_vgw && (var.enable_vpn_gateway || var.vpn_gateway_id != "") ? length(var.transit_subnets) : 0
