...
variable "transit_subnet_ipv6_prefixes" {
  description = "Assigns IPv6 transit subnet id based on the Amazon provided /56 prefix base 10 integer (0-256). Must be of equal length to the corresponding IPv4 subnet list"
  type        = list(string)
  default     = []
}
...
variable "transit_subnet_assign_ipv6_address_on_creation" {
  description = "Assign IPv6 address on transit subnet, must be disabled to change IPv6 CIDRs. This is the IPv6 equivalent of map_public_ip_on_launch"
  type        = bool
  default     = null
}
...
variable "appliance_subnets" {
  description = "A list of appliance subnets inside the VPC"
  type        = list(string)
  default     = []
}
...
variable "enable_network_firewall_routing" {
  description = "Should be true if you want to routing traffic go through network firewall for inspection"
  type        = bool
  default     = false
}

variable "network_firewall_endpoint_ids" {
  description = "The IDs of endpoint to routing to network firewall"
  type        = list(string)
  default     = []
}

variable "network_firewall_destination_cidr_block" {
  description = "Used to pass a custom destination route for network firewall. If not specified, the default 0.0.0.0/0 is used as a destination route."
  type        = string
  default     = "0.0.0.0/0"
}
...
variable "transit_gateway_id" {
  description = "ID of transit Gateway to attach to the VPC"
  type        = string
  default     = ""
}
...
variable "propagate_transit_route_tables_vgw" {
  description = "Should be true if you want route table propagation"
  type        = bool
  default     = false
}
...
variable "transit_route_table_tags" {
  description = "Additional tags for the transit route tables"
  type        = map(string)
  default     = {}
}
...
variable "transit_acl_tags" {
  description = "Additional tags for the transit subnets network ACL"
  type        = map(string)
  default     = {}
}
...
variable "transit_dedicated_network_acl" {
  description = "Whether to use dedicated network ACL (not default) and custom rules for transit subnets"
  type        = bool
  default     = false
}
...
variable "transit_inbound_acl_rules" {
  description = "Transit subnets inbound network ACLs"
  type        = list(map(string))
...
variable "transit_outbound_acl_rules" {
  description = "Transit subnets outbound network ACLs"
  type        = list(map(string))
